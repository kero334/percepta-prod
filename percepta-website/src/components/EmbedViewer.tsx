interface EmbedViewerProps {
  url: string;
  contentType: "video" | "presentation" | "pdf";
  title: string;
}

export default function EmbedViewer({ url, contentType, title }: EmbedViewerProps) {
  const getEmbedUrl = (originalUrl: string, type: string): string => {
    // YouTube URLs
  ...
  const getAspectRatio = () => {
    if (contentType === "video") return "56.25%"; // 16:9
    if (contentType === "presentation") return "75%"; // 4:3
    if (contentType === "pdf") return "141.42%"; // A4 ratio (1:1.414)
    return "56.25%";
  };
