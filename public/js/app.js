...
const App = {
    ...
    async init() {
        ...
        if (!this.state.visionReady) {
            UI.showToast('Vision model failed to load. Detection may be limited.', 'warning');
        }
        if (!this.state.backendReady) {
            UI.showToast('Backend server not reachable. AI analysis unavailable.', 'error');
        }
    ...
    async runAnalysis() {
        ...
        if (!this.state.backendReady) {
            UI.showToast('Live analysis service is currently unavailable. Please try again later.', 'error');
            return;
        }
        ...
        try {
            ...
            // Step 2: Object Detection (COCO-SSD — Tier 1)
            const cocoDetections = await DetectionEngine.detect(this.state.currentImage);
            this.state.detections = cocoDetections;
            UI.updateLoading(`Found ${cocoDetections.length} objects (Tier 1). Rendering...`, 30);
            ...
            this.state.proximityHazards = DetectionEngine.calculateProximityHazards(cocoDetections, canvasWidth);

            // Step 4: Render COCO-SSD detections on canvas
            DetectionEngine.renderOnCanvas(canvas, this.state.currentImage, cocoDetections, this.state.proximityHazards);
            UI.showCanvas();
            UI.updateLoading('Running AI deep analysis (detecting all entities)...', 50);

            // Step 5: AI Analysis via backend (Gemini reasoning only)
            const analysis = await AnalysisEngine.analyze(
                canvas, cocoDetections, this.state.proximityHazards, this.state.analysisMode
            );
            this.state.aiReport = analysis;
            UI.updateLoading('Processing AI detections...', 80);

            // Stage 2 must not draw bounding boxes; preserve YOLO-only canvas rendering
            this.state.detections = cocoDetections;
